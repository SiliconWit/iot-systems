...
Individual subsystems that work perfectly in isolation have a habit of falling apart when combined. A broker that handles three test clients chokes when TLS, Telegraf, Node-RED, and a Flask API all compete for connections. A dashboard that looks great on synthetic data shows nothing when real sensor payloads arrive with unexpected field names. Production readiness means every component has been tested together, failures are detected automatically, and the system recovers without human intervention. This capstone pulls together protocol selection, broker configuration, multi-MCU MQTT clients, time-series storage, REST APIs, alert automation, and device security into a single greenhouse monitoring deployment with three sensor nodes, live Grafana dashboards, Slack alerts, and cloud forwarding to SiliconWit.io. #IoT #Production #Capstone
...
```text
  ...
  docker-compose.yml
  ┌─────────────────────────────────────┐
  │ mosquitto    :1883, :8883 (TLS)     │
  │ telegraf     (internal, no port)    │
  │ influxdb     :8086                  │
  │ grafana      :3000                  │
  │ nodered      :1880                  │
  │ flask-api    :5000                  │
  └─────────────────────────────────────┘
         │
  Shared Docker network: iot-net
  Services reference each other by name:
    telegraf ──► mosquitto (mqtt://mosquitto)
    telegraf ──► influxdb (http://influxdb)
    grafana  ──► influxdb (http://influxdb)
    nodered  ──► mosquitto (mqtt://mosquitto)
```
...
:::tip[Instrument it]
Placement decides whether the readings are real. Put a capacitive soil moisture probe (not a resistive one, it corrodes) in the root zone, set back from the dripper so it reads the bed and not the last watering, and keep air temperature and humidity at canopy height in a small radiation shield, shaded from sun and clear of vents. Good, cheap parts: an SHT31 or BME280 for temperature and humidity, a BH1750 for light in lux (far better than a bare LDR), and an SCD41 if you want true CO2. An [ESP32](/education/sensor-actuator-interfacing-stm32/) reads them all over I2C, and once the node earns a small board it can do more than watch: [stream](/education/iot-systems/) to a live [dashboard](https://siliconwit.io) and close the loop, watering when the bed dries and venting when it overheats.
:::
